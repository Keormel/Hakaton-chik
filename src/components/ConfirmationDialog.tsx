...
export function ConfirmationDialog({
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm">
      <div className="relative w-full max-w-sm overflow-hidden rounded-2xl 
        ...
        {/* Content */}
        <div className="p-6">
          <p className="text-blue-800/80 dark:text-blue-200 mb-6">{message}</p>
          <div className="flex justify-end space-x-4">
            <button onClick={onClose} className="px-4 py-2 rounded-lg bg-white/40 dark:bg-gray-700/60 backdrop-blur-sm
              border border-white/30 dark:border-gray-600/50
              text-blue-800/80 dark:text-blue-200
              hover:bg-white/60 dark:hover:bg-gray-600/70 transition-all duration-200">
              {t('no')}
            </button>
            <button onClick={onConfirm} className="px-4 py-2 rounded-lg bg-gradient-to-r 
